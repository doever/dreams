...
1. 1.
   ...
   `**__name__ == "__main__"**`**：** 当一个 Python 脚本被直接执行时，`__name__` 的值为 `"__main__"`。这是常用于判断模块是被直接运行还是被导入到其他模块中的条件。
...
2. 2.
   ...
   **在模块中的其他情况：** 如果一个模块被导入到其他模块中，`__name__` 的值为模块的名称。
...
3. 3.
   ...
   **在交互式环境中的情况：** 在交互式环境中，`__name__` 的值为 `"__main__"`，与直接执行脚本时的情况相同。这样可以使得在交互环境中定义的函数或变量与脚本中的行为一致。
...
4. 4.
   ...
   **在 Python 脚本中运行的模块的情况：** 如果一个 Python 脚本中的代码块（而非被导入的模块）被执行，`__name__` 的值为 `"__main__"`。
